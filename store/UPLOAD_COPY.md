...
> ⚠️ **两个 Description 是不同字段,别搞混:**
>
> | 位置 | 字数限制 | 风格 |
> |---|---|---|
> | **GitHub 仓库描述**(github.com/new 建仓时填) | **≤350 字符** | 一句话简介,展示在仓库名下方 |
> | **扩展平台 Description**(extensions.blender.org 上传表单) | 未确认(无明确报错,可填长) | 功能 + 用法 + 兼容性的完整说明 |

---

## ① GitHub 仓库描述(github.com/new 的 Description 字段,≤350 字符)

**简短版(推荐):**
```
C4D/Maya-style weight painting panel for Blender — precise, brush-free weight editing in a side panel.
```

**详细版(更易被搜索):**
```
C4D/Maya-style weight painting panel for Blender. Precise brush-free weight editing in a side panel: drag sliders to set/smooth weights, mirror & copy/paste, normalize, per-vertex weight table, select-by-weight, live HUD, edge-loop picking in Weight Paint mode. Blender 4.2+.
```

---

## ② 扩展平台 Description(extensions.blender.org 上传表单)
